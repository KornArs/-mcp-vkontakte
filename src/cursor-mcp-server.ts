          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                owner_id: { type: 'string', description: 'ID владельца (опционально)' },
              },
              required: ['imageUrl'],
            },
          },
      ...
      try {
        switch (name) {
          ...
          case 'get_post_stats': {
            ...
            const result = await this.vkApi.getPostStats(postId, ownerId);
            return { content: [{ type: 'text', text: JSON.stringify(result) }] };
          }

          case 'get_wall_by_id': {
            const posts = String(args.posts);
            const result = await this.vkApi.getWallById(posts);
            return { content: [{ type: 'text', text: JSON.stringify(result) }] };
          }
